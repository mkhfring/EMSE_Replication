...
java = os.path.join(current_location, 'train_java_selected')
previous_java = os.path.join(current_location, 'java_selected')

java_problems = os.listdir(java)
ruby_problems = os.listdir(previous_java)
shared = [p for p in java_problems if p in ruby_problems]
data = []
...
random.shuffle(data)
with open(os.path.join(current_location, 'java_train_clones.jsonl'),'w') as f:
